...
export type { Group } from '../config/groups'
export type TaskStatus = '未开始' | '进行中' | '待验收' | '已完成' | '已逾期' | '已停滞' | '已放弃'
export interface Member { id: string; name: string; group: Group | null; active: boolean; joinedAt: string }
export interface Task { id: string; title: string; memberIds: string[]; group: Group | null; status: TaskStatus; priority: number; startAt?: string; deadline?: string; completedAt?: string; progress: number; description: string; category?: string; tags?: string[]; parentIds?: string[]; parentTitles?: string[] }
