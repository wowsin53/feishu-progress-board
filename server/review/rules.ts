...
export function validateTask(task: ReviewTask, records: Map<string, ReviewTask>, complete: boolean, creation?: CreationSnapshot, observation?: FirstObservation, mergeOnly = false): RuleResult {
  const parsed = parseTags(task.tags)
  const result: RuleResult = { group: parsed.groups.length === 1 ? parsed.groups[0] : null,
    robotTypes: parsed.robotTypes, issues: [], errors: task.errors.filter(e => !(task.parentRecordIds.length && e === 'INVALID_CELL:预计完成日期')), dateIssues: [] }
  ...
  if (!task.parentRecordIds.length && ['进行中', '已停滞'].includes(task.status) && task.dueDate === null) fail('DUE_EMPTY', '进行中或已停滞任务必须填写预计完成日期', true)
  if (!task.parentRecordIds.length && task.startDate !== null && task.dueDate !== null && task.startDate > task.dueDate) fail('DATE_ORDER', '开始日期不能晚于预计完成日期', true)
  if (!mergeOnly) {
