...
export interface AuditInput { record: RawRecord; records: RawRecord[]; complete: boolean; metadata: FieldMetadata[]; tableId: string; creation?: CreationSnapshot; observation?: FirstObservation; deferRelations?: boolean; auditRevision?: 'filled' }
export interface AuditResult {
  auditRevision?: 'filled'; observation?: FirstObservation; recordId: string; taskText: string; taskType: 'main' | 'child' | 'unknown'; reviewedAt: string
...
export class DryRunReviewer {
  ...
  run(input: AuditInput): Promise<AuditResult> {
    const key = `${input.tableId}:${input.auditRevision ? input.auditRevision + ":" : ""}${input.record.record_id}`
  ...
  private async review(input: AuditInput, key: string): Promise<AuditResult> {
    ...
    const out: AuditResult = { recordId: task.recordId, taskText: task.taskText, taskType: task.errors.includes('INVALID_PARENT_CELL') ? 'unknown' : task.parentRecordIds.length ? 'child' : 'main',
      reviewedAt: new Date().toISOString(), auditRevision: input.auditRevision, observation: input.observation, task, fingerprint: hash, threshold: this.config.rejectConfidence,
    ...
    try {
      ...
      if (out.decision !== 'SKIPPED_HISTORY' && !out.errors.length) {
        ...
        out.basic = validateTask(task, records, input.complete, input.creation, input.observation, input.deferRelations)
        ...
        else if (!out.errors.length && input.deferRelations) out.decision = 'PASS'
