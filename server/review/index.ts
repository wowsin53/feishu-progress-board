...
import { sendWebhook,webhookConfig } from '../reports/webhook'
...
const worker=new ReviewWorker(new ReviewApi(baseline.base,baseline.table),store,baseline,sendWebhook)
let stopping=false,timer:ReturnType<typeof setTimeout>|undefined
async function loop(){try{await worker.tick()}catch{console.error(JSON.stringify({event:'task_review',state:'scan_failed'}))}finally{if(!stopping)timer=setTimeout(loop,60000)}}
console.log(JSON.stringify({event:'task_review_started',since:baseline.since,excluded:baseline.recordIds.length,intervalSeconds:60}))
void loop()
