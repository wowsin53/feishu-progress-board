...
当前表的字段已适配：任务描述作为标题，任务负责人和任务执行人合并去重，进展作为状态，预计完成日期作为截止日期。多选组别优先识别机械／电控／视觉，重装、飞镖等标签保留；父记录关系保留，子任务以 ↳ 标记。只包含默认开始日期的空白行跳过。待开始显示为未开始；已停滞仍计入未完成；已放弃保留原始记录，不计入未完成、逾期与完成率分母。父子任务分别按独立记录统计。
...
显示电脑设置3840×2160，浏览器100%缩放，F11全屏或配置浏览器kiosk启动。顶部统计、时间与底部语录固定。成员列表、每人完整任务列表和预警列表分别自动滚动，首尾停留4秒，不缩小字号；鼠标滚轮或键盘查看时暂停自动滚动15秒后恢复。
