...
默认首页为专用常驻展示模式，按 **3840×2160、16:9** 原生画布布局。顶部统计、时钟、组别标题和底部语录固定；成员列表、每个人当前/已完成任务、缺卡及预警列表在各自区域自动滚动。首尾停留4秒，鼠标/键盘操作时暂停15秒。无须触屏或翻页，数据增长不再缩小字号。每秒更新时钟，每60秒同步数据，断线保留旧数据并自动重试。
...
- 当前16人、48项任务按原生4K排布。单人同时有15项或更多任务时，完整列表自动循环滚动；单组成员增多时整列成员滚动，字号与画布尺寸保持不变。
...
- 成员卡片：个人完成率、最多三个当前任务和三个最近完成任务；点击打开完整列表。
...
- 成员完成率 = 已完成任务数 ÷ 全部任务数；零任务显示「暂无任务」。当前任务优先级从高到低，同优先级按最近截止排序。
