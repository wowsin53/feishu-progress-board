...
宽屏首页按 3840×2160 原生画布布局，1920×1080 与 4K 均保持五组一行。顶部统计、时钟、组标题和 RM DAILY 固定；各组成员区域独立轮播，超出时每4秒向上切换一人，动画600ms，首尾无缝循环。悬停只暂停当前组，后台暂停且返回后重新计时。人数少时静止。右侧缺卡和预警继续自动滚动；每60秒同步，断线保留旧数据并重试。

界面截图仅保留在本地，不随源码提交。默认首页按视口自动选择布局：超过1024px使用4K大屏，1024px及以下使用手机／平板交互布局。
...
新增未填写截止日期独立告警和服务器端每日日报，发送时间默认为可配置的北京时间00:30，汇总前一天考勤及完成记录，任务预警按发送时快照计算。管理员可预览和手动发送，SQLite持久化发送记录并防止同日重复。配置与统计口径见 [日报说明](docs/daily-reports.md)。独立ECS部署使用 deploy/fs.service、deploy/fs.nginx.conf 和 deploy/install-release.sh。
...
手机直接打开首页即可，无需添加参数。宽度不超过1024px时使用交互看板，以正常字号显示双列统计卡、成员列表与预警；窄屏成员卡为单列。支持组别筛选、成员分页、任务详情和手动刷新，页面可纵向触摸滚动。未打卡、逾期、即将到期、缺截止日期及每日语录继续保留，虚拟打卡明确标注。横竖屏或窗口跨过断点时自动切换布局。

1920×1080及4K大屏继续使用五组独立轮播。显式 `?view=desktop` 仍固定使用交互看板，`?view=admin` 仍进入日报管理页。前后端接口和飞书读取配置不变。

### 日报缺截止日期口径

- 日报“未填写截止日期的负责人”和“今日重点”的缺日期任务数，只统计**非待开始的未完成任务**。
- 飞书“待开始”经现有读取逻辑归一为“未开始”，从该项日报统计中排除；进行中、待验收、已停滞等未完成状态仍计入。
- 已完成、已放弃继续排除；负责人只列姓名并去重，多人负责的任务在任务总数中只算一项。
- 看板的缺截止日期提示保持原口径，仍展示未开始但缺日期的任务；其他日报统计和发送时间不变。
- 不需要修改飞书字段。推送GitHub不会自动更新阿里云ECS；生产部署需另行确认。

验证命令：`npm test`、`npm run build`；启动本地Vite后运行 `npm run test:ui`，覆盖手机首页、横竖屏、布局切换及原有大屏交互。

### 多人跨组任务的人员归组

任务负责人和任务执行人合并去重后，同一任务显示在每位参与者的卡片内。成员按各自组别展示；各组统计可同时包含协作任务，基地总数按任务ID去重。

有成员表时，以成员表的组别为准。只有任务表且多人任务同时标了机械、电控时，不能按任务标签或人员顺序猜归属。可先查询飞书通讯录核实部门，再在服务器环境变量配置人员ID到组别的映射：

```dotenv
FEISHU_MEMBER_GROUPS_JSON='{"ou_example_electrical":"电控组","ou_example_mechanical":"机械组"}'
```

以上为占位示例；实际ID与已核实归组只保存在未提交的环境配置中。此配置优先于任务推断，未配置人员沿用既有推断；仍不能确定则保留待分组。当前为经通讯录核实后维护的映射，不会在每次刷新时请求通讯录；调组后需更新配置或成员表。部署时需同步该环境变量并重启后端，生产修改须先确认。

日报末尾固定附上完整任务看板网址：http://47.93.156.196/ ，手机和电脑均可打开；超长日报缩减任务详情时仍保留该链接。

2026-09-19 已核对GIRT基地群90名成员的人员资料，确认五组归属76人（机械37、电控17、视觉16、运营6，未查到操作手组成员）。仅保存人员ID到组别的对应关系，填写任务后才展示；另14人保留原部门，不强制归入五组。真实名单及配置不提交Git。群成员范围不等同于租户全员，新增成员或调组后需更新映射。

## 新任务审核与通知（已上线，不删除）

确定性规则、候选检索、DeepSeek Provider 和只读 DRY_RUN 引擎见 [开发说明](docs/review-v2.md)。
默认不删除、不发送成员通知。现已增加按「填写时间（系统）」轮询的只读入口，默认每 15 秒检查；固定启用时间、SQLite 幂等，首次读到终态转人工确认。已兼容真实父记录嵌套 ID；占位对象单条复查后仍不明则保留、转人工检查。已于北京时间 2026-09-21 22:43:45 启用独立 fs-review-v2 服务；每轮完成后等待 15 秒，实际间隔包含 API 读取耗时。只审核该启用时间之后创建的记录。配置、限制与模型验证命令见说明。

新增“审核＋通知、不删除”模式：DeepSeek 独立判断内容清晰度；结果私聊系统填写人并抄送配置的武珊。父记录不明仍保留人工确认，不影响内容建议。通过 REVIEW_NOTIFY_ENABLED 显式启用，使用持久化消息去重及有限重试，详见审核开发说明。

2026-09-22 已启用自动通知：系统填写人收到审核结果，同时抄送武珊（同人去重）。真实新任务的 DeepSeek 内容判断和飞书成功发送回执已验证；不会删除或修改飞书任务记录。

最新触发规则已实现：所有任务等七项（任务描述、负责人、执行人、组别、进展、开始日期、重要紧急程度）填齐后审核。填写父记录的任务跳过截止日期检查；任务本身问题优先于并入建议。父子关系不明不再发送人工确认提示；已于 2026-09-22 13:04 部署生效，不删除任务。

### 日报末尾的主任务填写入口

日报保留任务看板网址，并在最后附上「主任务填写」表单的真实分享链接：https://girtrobotlab.feishu.cn/share/base/shrcndaRaDyprWeJ1UJ8cJjrxgg 。2026-09-24 已通过飞书 form-share-get 核实该链接属于现有主任务表单，分享处于启用状态；超长日报缩减详情时仍保留两个入口。

日报继续由服务器每天北京时间 00:30 调度，保留 SQLite 发送去重及历史。若群自定义机器人被删除，重新部署不能恢复它；需在群内新建自定义机器人，将新 Webhook/签名配置保存至服务器秘密环境变量，再验证发送。不要清空日报历史或提交机器人密钥。
