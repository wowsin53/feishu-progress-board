...
if (!Number.isInteger(runs) || runs < 1 || runs > 5) throw new Error('RUNS_MUST_BE_1_TO_5')
...
if (!config.apiKey || !config.baseUrl || !config.model) {
  ...
  for (let run = 1; run <= runs; run++) for (const sample of semanticCases) {
    ...
    try {
      ...
      console.log(JSON.stringify({ run, case: sample.name, expected: sample.expected, ...result, matched, deleted: false }))
